...
export async function login(
  ...
  writeAuditLog({
    ...
    actor: `user:${org.email}`,
    action: 'auth.login',
    resourceType: 'organization',
    resourceId: org.id,
    metadata: { totpUsed: org.totp_enabled },
  })
...
export async function handleGoogleCallback(code: string): Promise<{
  ...
  if (byGoogleId[0]) {
    writeAuditLog({
      ...
      actor: `user:${byGoogleId[0].email}`,
      action: 'auth.login',
      resourceType: 'organization',
      resourceId: byGoogleId[0].id,
  ...
  if (byEmail[0]) {
    ...
    writeAuditLog({
      ...
      actor: `user:${byEmail[0].email}`,
      action: 'auth.login',
      resourceType: 'organization',
      resourceId: byEmail[0].id,
      metadata: { provider: 'google', linkedGoogleId: true },
    })
    return {
      organizationId: byEmail[0].id,
  ...
  writeAuditLog({
    organizationId: row.id,
    actor: `user:${googleUser.email}`,
    action: 'auth.register',
    resourceType: 'organization',
    resourceId: row.id,
    metadata: { provider: 'google', name: googleUser.name },
  })
...
export async function handleGitHubCallback(code: string): Promise<{
  ...
  if (byGitHubId[0]) {
    writeAuditLog({
      ...
      actor: `user:${byGitHubId[0].email}`,
      action: 'auth.login',
      resourceType: 'organization',
      resourceId: byGitHubId[0].id,
  ...
  if (byEmail[0]) {
    ...
    writeAuditLog({
      ...
      resourceType: 'organization',
      resourceId: byEmail[0].id,
      metadata: { provider: 'github', linkedGitHubId: true },
    })
    return {
      organizationId: byEmail[0].id,
      email: byEmail[0].email,
  ...
  writeAuditLog({
    ...
    resourceType: 'organization',
    resourceId: row.id,
    metadata: { provider: 'github', name: displayName },
  })
 ...
 * Logs the reset URL to console (email sending not implemented yet).
 */
export async function forgotPassword(email: string): Promise<void> {
  ...
  `
  const org = rows[0]
  if (!org) return // don't leak existence
  ...
  // TODO: send email — for now just log the reset URL
  const resetUrl = `https://verify.solidus.network/reset-password?token=${token}`
...
export async function resetPassword(token: string, newPassword: string): Promise<void> {
  ...
  const rows = await sql<{ id: string; reset_token_expires_at: Date }[]>`
  ...
  if (!org || !org.reset_token_expires_at || org.reset_token_expires_at < new Date()) {
