...
export async function createEndpoint(
  ...
  writeAuditLog({
    ...
    resourceId: row.id,
    metadata: { url, events: events.length > 0 ? events : ['verification.completed', 'credential.issued'] },
  })
...
export async function updateEndpoint(
  ...
  // Build parameterized update using the postgres sql() dynamic helper
  ...
  if (patch.events !== undefined) updates.events = sql.array(patch.events)
