    ...
    <div className="p-8 max-w-[1400px] mx-auto w-full flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              const headers = ['ID', 'DID', 'Type', 'Status', 'Country', 'Duration', 'Cost', 'Timestamp']
              const csvRows = [headers.join(',')]
              filtered.forEach((r) => {
                csvRows.push([r.id, r.did, r.type, r.status, r.country, r.duration, r.cost, r.time].map(v => `"${v}"`).join(','))
              })
            ...
            className="h-9 px-4 rounded-md border border-border text-[14px] font-medium text-white hover:bg-elevated transition-colors flex items-center gap-2"
          >
          ...
          <div className="h-[60px] px-5 flex items-center justify-between border-t border-elevated bg-surface">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className={`h-7 px-2 text-[12px] font-medium ${page <= 1 ? 'text-text-disabled cursor-not-allowed' : 'text-white hover:text-cta transition-colors'}`}
              >Previous</button>
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  className={`w-7 h-7 rounded-full text-[12px] font-medium flex items-center justify-center ${n === page ? 'bg-cta text-white' : 'text-text-secondary hover:bg-elevated hover:text-white transition-colors'}`}
                >{n}</button>
              ))}
              {totalPages > 5 && (
                <>
                  ...
                  <button
                    ...
                    className={`w-7 h-7 rounded-full text-[12px] font-medium flex items-center justify-center ${totalPages === page ? 'bg-cta text-white' : 'text-text-secondary hover:bg-elevated hover:text-white transition-colors'}`}
                  >{totalPages}</button>
                </>
              )}
              <button
                ...
                className={`h-7 px-2 text-[12px] font-medium ${page >= totalPages ? 'text-text-disabled cursor-not-allowed' : 'text-white hover:text-cta transition-colors'}`}
              >Next</button>
            </div>
          </div>
        )}
