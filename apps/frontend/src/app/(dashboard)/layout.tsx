...
import { SandboxProvider, useSandbox } from '../../lib/sandbox-context'
import {
  BarChart2,
  Key,
  ScrollText,
...
function Sidebar({ open, onClose }: { open?: boolean; onClose?: () => void }) {
  const { isSandbox, toggle } = useSandbox()
  ...
  return (
    <>
      ...
      <div className={`
        fixed inset-y-0 left-0 z-50 w-[240px] bg-surface border-r border-border flex flex-col h-screen
      ...
      <div className="h-16 px-5 flex items-center justify-between shrink-0">
        ...
        <button
          onClick={onClose}
          className="md:hidden w-8 h-8 flex items-center justify-center rounded-md hover:bg-elevated transition-colors"
        >
...
function TopBar({ onMenuClick }: { onMenuClick?: () => void }) {
  return (
    <div className="h-16 px-4 md:px-6 bg-surface border-b border-border flex items-center justify-between sticky top-0 z-10">
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="md:hidden w-9 h-9 rounded-md bg-surface border border-border flex items-center justify-center hover:bg-elevated transition-colors duration-150"
        >
        ...
        <div className="font-semibold text-base md:hidden">Solidus Verify</div>
      </div>
      <div className="flex items-center gap-3">
