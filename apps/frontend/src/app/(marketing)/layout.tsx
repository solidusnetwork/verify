      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3 mt-2">
            {[
              { Icon: Linkedin, href: 'https://linkedin.com/company/solidusnetwork' },
              { Icon: Twitter, href: 'https://x.com/solidusnetwork' },
              { Icon: Github, href: 'https://github.com/solidusnetwork' },
              { Icon: MessageSquare, href: 'mailto:hello@solidus.network' },
            ].map(({ Icon, href }) => (
              <a key={href} href={href} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded bg-elevated hover:bg-elevated/80 flex items-center justify-center transition-colors">
        ...
        {[
          { title: 'Product', links: [
            ...
            { label: 'Status', href: 'https://explorer.solidus.network' },
          ]},
          { title: 'Developers', links: [
            { label: 'Documentation', href: 'https://docs.solidus.network' },
            { label: 'API Reference', href: 'https://docs.solidus.network/api' },
            { label: 'SDKs', href: 'https://github.com/solidusnetwork/sdk' },
            { label: 'Quickstart', href: 'https://docs.solidus.network/quickstart' },
            { label: 'Webhook Guide', href: 'https://docs.solidus.network/webhooks' },
            { label: 'Sample Code', href: 'https://github.com/solidusnetwork/sdk' },
            { label: 'Rate Limits', href: 'https://docs.solidus.network/api#rate-limits' },
          ]},
          { title: 'Company', links: [
            { label: 'About Solidus', href: 'https://solidus.network/about' },
            { label: 'Pricing', href: '/pricing' },
            { label: 'Security & Trust', href: '/security' },
            { label: 'Privacy Policy', href: 'https://solidus.network/privacy' },
            { label: 'Terms of Service', href: 'https://solidus.network/terms' },
            { label: 'GDPR', href: 'https://solidus.network/privacy' },
            { label: 'Contact Us', href: 'mailto:hello@solidus.network' },
          ]},
        ].map(col => (
          <div key={col.title} className="flex flex-col gap-4">
            ...
            {col.links.map(l => (
              l.href.startsWith('http') || l.href.startsWith('mailto')
                ? <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer" className="text-[14px] text-text-secondary hover:text-text-primary transition-colors">{l.label}</a>
                : <a key={l.label} href={l.href} className="text-[14px] text-text-secondary hover:text-text-primary transition-colors">{l.label}</a>
            ))}
          </div>
        ))}
      </div>
