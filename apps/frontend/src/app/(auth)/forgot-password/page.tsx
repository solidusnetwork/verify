...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend always returns ok, but handle network errors gracefully
      setSent(true)
    } finally {
      setSubmitting(false)
    }
        ...
        <div className="w-full bg-surface rounded-xl border border-border p-6 flex flex-col gap-4">
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button type="submit" disabled={submitting} className="h-12 w-full bg-cta hover:bg-cta/90 text-white text-[16px] font-semibold rounded-md transition-colors shadow-[0_0_12px_rgba(0,102,255,0.4)] disabled:opacity-50 disabled:cursor-not-allowed">{submitting ? 'Sending...' : 'Send Reset Link'}</button>
            </form>
          )}
        </div>
